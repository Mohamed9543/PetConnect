...
export default function ReportFound() {
  ...
  const onSubmit = async () => {
    ...
    if (!coords) {
      showToast(t("reportForm.locationRequiredErrorFound"), "error");
      return;
    }
    setSubmitting(true);
    try {
      ...
      showToast(t("reportForm.foundSubmitSuccess", { reference: created.reference }), "success");
      router.back();
    } catch (error) {
  ...
  return (
    <ScrollView className="flex-1 bg-surface" contentContainerStyle={{ padding: 20 }}>
      <View className="flex-row items-center mb-1">
        ...
        <Text className="text-[20px] font-bold text-ink ml-2">{t("reportForm.foundHeader")}</Text>
      </View>
      <Text className="text-ink-secondary text-[13px] mb-5">
        {t("reportForm.foundSubheading")}
      </Text>
      ...
      <Input label={t("reportForm.breedIfKnownLabel")} placeholder={t("reportForm.breedPlaceholderCat")} value={form.breed} onChangeText={(v) => update("breed", v)} />
      ...
      <Text className="text-ink-secondary text-[13px] font-semibold mb-1.5">{t("reportForm.genderIfVisible")}</Text>
      <View className="flex-row mb-4" style={{ gap: 8 }}>
        {[
      ...
      <Input label={t("reportForm.dateFoundLabel")} leftIcon="calendar-outline" value={form.date} onChangeText={(v) => update("date", v)} />

      <Text className="text-ink-secondary text-[13px] font-semibold mb-1.5">{t("reportForm.locationFoundLabel")}</Text>
      <Pressable
      ...
      <Input
        ...
        placeholder={t("reportForm.descriptionPlaceholderFound")}
      ...
      <Button title={t("reportForm.foundSubmitButton")} onPress={onSubmit} loading={submitting} style={{ marginTop: 8, marginBottom: 24 }} />
    </ScrollView>
  );
}
